...
- **Credentials.** For private repositories, `spec.repository.secretRef` names a Secret mounted read-only at `/var/run/secrets/langop.io/git` into the `repository` init container and the agent container. The operator never reads it. Git is configured through `GIT_CONFIG_COUNT`/`GIT_CONFIG_KEY_n`/`GIT_CONFIG_VALUE_n` in both containers: a default identity (`user.name` = the agent name, `user.email` = `<name>@<namespace>.langop.io`) and, for HTTPS URLs, a credential helper scoped to the repository host that reads `username` (default `x-access-token`) and `password` or `token` from the mount; SSH URLs get `GIT_SSH_COMMAND` pointing at `ssh-privatekey`. `GIT_TERMINAL_PROMPT=0` makes a missing credential fail fast. So a runtime can `git fetch` and `git push` without any setup of its own.
- **Vendor CLI.** `spec.repository.vendor` (`github`, `gitlab` or `git`, defaulted from the host) selects which CLI receives the Secret's `token` key in the agent container: `GH_TOKEN` (and `GH_HOST` off github.com) for `gh`, `GITLAB_TOKEN` (and `GITLAB_HOST` off gitlab.com) for `glab`. The reference is optional, so Secrets without a `token` key still start. Runtimes that want this must ship the CLI in their image.
...
# uuid, clusterName, clusterUUID are injected as env vars (AGENT_UUID, etc.), not written here.
agent:
  name: data-analyst
  namespace: default
