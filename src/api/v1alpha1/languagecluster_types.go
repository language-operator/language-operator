...
type ClusterOIDCSpec struct {
	...
	// oauth2-proxy performs standard OIDC discovery against this issuer's
	// /.well-known/openid-configuration endpoint, so the issuer must be reachable
	// from inside the cluster; add an egress NetworkPolicy rule for it if
	// spec.networkPolicies.egress restricts egress.
