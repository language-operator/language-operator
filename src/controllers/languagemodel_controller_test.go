...
// TestLanguageModelController_NoOwnedResources verifies that the LanguageModel
// controller does not create any Deployments, Services, or ConfigMaps — those are
// the cluster proxy's responsibility.
func TestLanguageModelController_NoOwnedResources(t *testing.T) {
	...
	if err := fakeClient.Get(ctx, types.NamespacedName{Name: GenerateConfigMapName(model.Name, "model"), Namespace: model.Namespace}, cm); err == nil {
		t.Error("LanguageModel controller should not create a per-model ConfigMap; the cluster controller builds gateway-config from CRs directly")
	}
}
