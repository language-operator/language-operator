...
Declare what you want in a manifest; the operator reconciles it into a WorkflowTemplate, the Workflow or CronWorkflow that runs it, a Service, and a NetworkPolicy, injects the model and tool endpoints, mounts the instruction and persona config, and keeps it that way as your cluster changes.
