...
module Based
  ...
  module Shell
    ...
    #
    # @example Run safe command
    #   Shell.run('ls', '-la', '/tmp')
    #   # => { success: true, output: "...", error: "", exitcode: 0, timeout: false }
    #
    # @example With user input (safe from injection)
    #   Shell.run('grep', user_input, '/etc/hosts')
    ...
    def self.capture!(cmd, *args, **options)
      ...
      unless result[:success]
        raise "Command failed (exit #{result[:exitcode]}): #{result[:error]}"
      end
      result[:output]
    end
