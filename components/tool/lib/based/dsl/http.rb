...
module Based
  ...
  class HTTP
    ...
    def self.build_http(uri, timeout: 30)
    ...
    def self.add_headers(request, headers)
      ...
      request['User-Agent'] ||= 'BASED-MCP/1.0'
    ...
    def self.add_auth(request, auth)
      case auth[:type]
      when :basic
    ...
    def self.execute_request(http, request, follow_redirects: false)
