...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`🔍 Starting persona watch for organization ${organization.name}${clusterName ? ` (cluster: ${clusterName})` : ''}`)
    ...
    const startWatch = async () => {
      ...
      if (request.signal.aborted || !isActive()) {
        console.log('🛑 Client disconnected, not starting persona watch')
        return
      }
        ...
        watchCleanup = await watchService.watchLanguagePersonas(
          ...
          (event: WatchEvent) => {
            try {
              ...
              console.error('⚠️  Error processing persona watch event:', error)
          ...
          (error: Error | null) => {
            ...
            if (!request.signal.aborted && isActive()) {
              console.log('🔄 Reconnecting persona watch immediately...')
    ...
    console.error('Unhandled error in persona watch route:', error)
    return new Response(
