...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`🔍 Starting tools watch for organization ${organization.name}`)
    ...
    const stream = new ReadableStream({
      start(controller) {
        ...
        const startWatch = async () => {
          try {
            watchCleanup = await watchService.watchLanguageTools(
              {
              ...
              (event: WatchEvent) => {
                ...
                console.log(`📡 Tool watch event: ${event.type} - ${event.object?.metadata?.name || 'unknown'}`)
                sendEvent(clientEvent, 'resource-update')
              },
              (error: Error) => {
                console.error('Tool watch error:', error)
                sendEvent({
                  type: 'error',
            ...
            console.error('Failed to start tool watch:', error)
            sendEvent({
              type: 'error',
        ...
        request.signal.addEventListener('abort', () => {
          console.log('🛑 Tool watch client disconnected')
          if (watchCleanup) {
            watchCleanup()
          }
          controller.close()
      ...
      cancel() {
        console.log('🛑 Tool watch stream cancelled')
    ...
    console.error('Error setting up tool watch:', error)
    return new Response(
      `Error setting up tool watch: ${error instanceof Error ? error.message : 'Unknown error'}`,
      { status: 500 }
    )
  }
}