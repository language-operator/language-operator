...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`🔍 Starting tool watch for organization ${organization.name}${clusterName ? ` (cluster: ${clusterName})` : ''}`)
    ...
    const startWatch = async () => {
      ...
      if (request.signal.aborted || !isActive()) {
        console.log('🛑 Client disconnected, not starting tool watch')
        return
      }
        ...
        watchCleanup = await watchService.watchLanguageTools(
          ...
          (event: WatchEvent) => {
            try {
              ...
              console.error('⚠️  Error processing tool watch event:', error)
    ...
    console.error('Unhandled error in tool watch route:', error)
    return new Response(
